...
[you might not need an effect](<[https://beta.reactjs.org/learn/you-might-not-need-an-effect](https://beta.reactjs.org/learn/you-might-not-need-an-effect)>).
...
This summed up which made me realize I need to
[adjust my best practices for react](<[https://twitter.com/Mkuehb/status/1542426932448567296](https://twitter.com/Mkuehb/status/1542426932448567296)>).
In this blog post I’m gonna share everything I thought I did right when using
React. But noticed now, it can solve them in a better way, according to the new
react docs.
...
This is, again, unecessary and inefficient. We shouldn’t use useEffect to cache
something like that. We don’t need to! We can remove the state, the whole
useEffect and once more calucalte this value during render!
...
Fetch the data in the parent pass it down, as in React, data flows best from top
to bottom. Makes error tracing a lot easier. Even though there were some
arguments for the child data fetching, it’s always better to lift it up.
...
The new react docs have a lot more example of situations where you might not
need an effect, with better examples and even some challenges for you. It’s
awesome that they took the time to write something so detailed and helpful. I’m
glad I could rethink some patterns which I thought were the best practices.

Grab a cup of coffee (or your favorit beverage) and take your time to read
through it. It’s so worth it and I learned a lot.
